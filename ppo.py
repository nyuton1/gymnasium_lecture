...
実行例:
    python ppo.py                      # 学習(2000step) → ベストモデル録画・再生
    python ppo.py --timesteps 50000    # 学習ステップ数を増やす
...
LOG_DIR = "./ppo_logs_bipedalwalkerhardcore/"             # ログ・モデルの保存先
VIDEO_FOLDER = "ppo-bipedalwalkerhardcore_videos_practice"  # 再生動画の保存先
FINAL_MODEL = "ppo_bipedalwalkerhardcore"                 # 学習後の最終モデル保存名(.zip)
...
def play() -> None:
    ...
    if os.path.exists(best_model_path + ".zip"):
        ...
        raise FileNotFoundError(
            "学習済みモデルが見つかりません。先に `python ppo.py --mode train` を実行してください。"
        )
