...
StableBaselines3 を用いて、9 つの深層強化学習アルゴリズム
**A2C / DDPG / TD3 / SAC / TRPO / PPO / RecurrentPPO / TQC / CrossQ** を Gymnasium の
`BipedalWalkerHardcore-v3` 環境で「学習 → ベストモデル保存 → 録画 → 再生」する
サンプル集です。`tqc.py` / `crossq.py` は「ハードモードで**最速ゴール**」を狙う追加スクリプトで、
前進速度ボーナス（`--speed-coef`）と所要時間計測（`measure_goal_time`）を備えます。
...
# 最速ゴール狙い（オフポリシー / sb3_contrib。sac.py と同じ run 構造 + 速度ボーナス・所要時間計測）
python tqc.py             # TQC    （本環境の実績トップ）
python crossq.py          # CrossQ （標本効率の高い新手法。sb3_contrib 2.4.0+）
```

> **TRPO / RecurrentPPO / TQC / CrossQ は拡張パッケージ `sb3_contrib` を使います**（`requirements.txt` に同梱済み）。
...
> `tqc.py` / `crossq.py` は `sac.py` と同じタイムスタンプ別 run 構造で、再生時に
> `measure_goal_time` がゴール所要時間（秒）と成功率を表示します（目標 ~17 秒 ≒ 850 step ＠50FPS）。
> `learning_starts=10000` のため、動作確認は `--timesteps 12000` 以上で行ってください。
...
| `--speed-coef F` | （`tqc.py` / `crossq.py` のみ）前進速度ボーナス係数。`0` で無効 | `0.3` |
| `--max-episodes N` | （`tqc.py` / `crossq.py` のみ）学習を止める総エピソード数の上限 | `5000` |

> `sac.py` / `tqc.py` / `crossq.py` はさらに進捗動画オプション `--progress-video-every` /
> `--no-progress-video` を持ちます（`--run` は全スクリプト共通。下の「生成されるファイル」参照）。
...
| `runs/<algo>/<run_id>/videos/progress/*.mp4` | 学習中の進捗動画（**SAC / TQC / CrossQ のみ**） |
| `tensorboard/` | TensorBoard 用ログ（全アルゴリズム共通） |

`<algo>` は `a2c` / `ddpg` / `td3` / `sac` / `trpo` / `ppo` / `recurrent_ppo` / `tqc` / `crossq`。
...
├── tqc.py             # TQC          （オフポリシー / sb3_contrib・最速ゴール狙い）
└── crossq.py          # CrossQ       （オフポリシー / sb3_contrib・最速ゴール狙い）
```
