...
実行例:
    ...
    python tqc.py --timesteps 90000000 --n-envs 8 --max-episodes 100000000 \
        --eval-freq 100000 --save-freq 1000000 --mode train   # 長時間学習（並列・評価間引き）
...
def train(
    ...
    #    既定値(eval=500 / save=1000 env-step)はスモーク用。長時間学習では
    #    --eval-freq / --save-freq で大きく緩める（直列評価が学習を圧迫しないように）。
    # -------------------------------------------------------------------------
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=500,
        help="評価する env-step 間隔（既定 500=スモーク用。長時間学習は 100000 等に緩める。内部で // n_envs）",
    )
    parser.add_argument(
        "--save-freq",
        type=int,
        default=1000,
        help="チェックポイント保存の env-step 間隔（既定 1000=スモーク用。長時間学習は 1000000 等に。内部で // n_envs）",
    )
    parser.add_argument(
