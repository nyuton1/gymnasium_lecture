"""TRPO (Trust Region Policy Optimization) を BipedalWalkerHardcore-v3 で学習・再生するスクリプト。

TRPO は方策更新の大きさを KL ダイバージェンスで測った「信頼領域(trust region)」内に
制約することで、1 回の更新で方策が壊れないよう保証するオンポリシー手法です。
PPO はこの考え方をクリッピングで簡略化したもので、TRPO はその源流にあたります。
（TRPO は stable-baselines3 本体ではなく拡張パッケージ sb3_contrib に実装されています）

実行例:
    python trpo.py                      # 学習(2000step) → ベストモデル録画・再生
    python trpo.py --timesteps 50000    # 学習ステップ数を増やす
    python trpo.py --mode play          # 学習をスキップし、保存済みモデルを再生
    python trpo.py --mode train         # 学習のみ（録画・再生しない）
...
# =============================================================================
# 設定（元 Colab ノートブック 11_ の TRPO セルから移植）
# =============================================================================
ENV_ID = "BipedalWalkerHardcore-v3"                        # 使用する Gym 環境
LOG_DIR = "./trpo_logs_bipedalwalkerhardcore/"             # ログ・モデルの保存先
VIDEO_FOLDER = "trpo-bipedalwalkerhardcore_videos_practice"  # 再生動画の保存先
FINAL_MODEL = "trpo_bipedalwalkerhardcore"                 # 学習後の最終モデル保存名(.zip)


def train(timesteps: int, n_envs: int, fall_penalty: float) -> None:
    """TRPO モデルを学習し、チェックポイント・ベストモデル・最終モデルを保存する。"""
    # -------------------------------------------------------------------------
    # 1. ログディレクトリの準備
    # -------------------------------------------------------------------------
    ...
    #    TRPO は実効バッチ = n_steps × n_envs で、n_envs を増やすほど更新が安定する。
    # -------------------------------------------------------------------------
    ...
    #    gae_lambda: GAE（一般化アドバンテージ推定）の λ
    # -------------------------------------------------------------------------
    model = TRPO(
        policy="MlpPolicy",
        env=vec_env,
...
def play() -> None:
    ...
    if os.path.exists(best_model_path + ".zip"):
        ...
        raise FileNotFoundError(
            "学習済みモデルが見つかりません。先に `python trpo.py --mode train` を実行してください。"
        )

    agent = TRPO.load(model_path)
    record_agent_video(agent, ENV_ID, VIDEO_FOLDER, deterministic=True)


def main() -> None:
    parser = argparse.ArgumentParser(description="TRPO を BipedalWalkerHardcore-v3 で学習・再生")
